...
class Profile extends Component<ProfilePropType> {
  ...
  state = {
    tabs: {
      index: 0,
      routes: [
        { key: '1', title: 'Mesajlar', icon: 'inbox' },
        { key: '2', title: 'Bildirimler', icon: 'notifications-active' },
        { key: '3', title: 'Aktivitelerim', icon: 'list' },
        { key: '4', title: 'Ayarlar', icon: 'settings' },
      ],
    },
  }
  ...
  renderScene = ({ route: { key } }: any): React$Element< * > => {
    ...
    switch (key) {
      case '1':
        return <Messages containerStyle={styles.sceneContainer} />
      case '2':
        return <Notifications containerStyle={styles.sceneContainer} />
      case '3':
